...
let coinbaseLog = log('tribeca:gateway:coinbase-api');

let keepaliveAgent = new HttpsAgent();

export let PublicClient = function(apiURI?: string) {
    let self = this;
...
export let AuthenticatedClient = function(key: string, b64secret: string, passphrase: string, apiURI?: string) {
    let self = this;
...
_.assign(AuthenticatedClient.prototype, new function() {
    ...
    prototype.request = function(method, uriParts, opts, callback) {
        ...
        let relativeURI = self.makeRelativeURI(uriParts);
        _.assign(opts, {
            'method': method,
        ...
        let what = timestamp + method + relativeURI + (opts.body || '');
        let key = new Buffer(self.b64secret, 'base64');
...
export let OrderBook = function(productID: string, websocketURI: string, restURI: string, timeProvider: Utils.ITimeProvider) {
    let self = this;
...
_.assign(OrderBook.prototype, new function() {
    ...
    prototype.changeState = function(stateName) {
        let self = this;
        let newState = self.STATES[stateName];
        ...
        let sc = { 'old': oldState, 'new': newState };
    ...
    prototype.onMessage = function(datastr: string) {
        let self = this;
        let t = self.timeProvider.utcNow();
    ...
    prototype.loadSnapshot = function(snapshotData) {
        ...
        let load = function(data) {
            let i, bid, ask;
            let convertSnapshotArray = function(array) {
