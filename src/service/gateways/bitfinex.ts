...
class BitfinexMarketDataGateway implements Interfaces.IMarketDataGateway {
    ...
    private downloadMarketTrades = () => {
        let qs = { timestamp: this._since === null ? moment.utc().subtract(60, 'seconds').unix() : this._since };
        this._http
    ...
    private onMarketData = (book: Models.Timestamped<BitfinexOrderBook>) => {
        let bids = BitfinexMarketDataGateway.ConvertToMarketSides(book.data.bids);
        let asks = BitfinexMarketDataGateway.ConvertToMarketSides(book.data.asks);
...
class BitfinexOrderEntryGateway implements Interfaces.IOrderEntryGateway {
    ...
    private downloadOrderStatuses = () => {
        let tradesReq = { timestamp: this._since.unix(), symbol: this._symbolProvider.symbol };
        this._http
...
class BitfinexHttp {
    ...
    private postOnce = <TRequest, TResponse>(actionUrl: string, msg: TRequest): Q.Promise<Models.Timestamped<TResponse>> => {
        ...
        let signature = crypto.createHmac('sha384', this._secret).update(payload).digest('hex');
