...
let SortedArrayMap = require('collections/sorted-array-map');
...
class CoinbaseOrderBook {
    ...
    private addToOrderBook = (storage: any, price: number, size: number, order_id: string) => {
        let priceLevelStorage: PriceLevel = storage.get(price);
    ...
    public onReceived = (msg: CoinbaseReceived, t: Date): boolean => {
        ...
        let price = convertPrice(msg.price);
        let size = convertSize(msg.size);
        let side = convertSide(msg);

        let otherSide = side === Models.Side.Bid ? Models.Side.Ask : Models.Side.Bid;
        let storage = this.getStorage(otherSide);
        ...
        for (let i = 0; i < storage.store.length; i++) {
            ...
            let kvp = storage.store.array[i];
            let price_level = kvp.key;

            if (side === Models.Side.Bid && price < price_level) break;
            if (side === Models.Side.Ask && price > price_level) break;

            let level_size = kvp.value.marketUpdate.size;
    ...
    public onOpen = (msg: CoinbaseOpen, t: Date) => {
        let price = convertPrice(msg.price);
        let side = convertSide(msg);
        let storage = this.getStorage(side);
    ...
    public onDone = (msg: CoinbaseDone, t: Date): boolean => {
        ...
        let orderSize = priceLevelStorage.orders[msg.order_id];
    ...
    public onMatch = (msg: CoinbaseMatch, t: Date): boolean => {
        ...
        let makerStorage = this.getStorage(side);
        let priceLevelStorage = makerStorage.get(price);
    ...
    public onChange = (msg: CoinbaseChange, t: Date): boolean => {
        ...
        let priceLevelStorage: PriceLevel = storage.get(convertPrice(msg.price));

        if (typeof priceLevelStorage === 'undefined')
            return false;

        let oldSize = priceLevelStorage.orders[msg.order_id];
        if (typeof oldSize === 'undefined')
            return false;

        let newSize = convertSize(msg.new_size);
...
class CoinbaseMarketDataGateway implements Interfaces.IMarketDataGateway {
    ...
    private onStateChange = (s: StateChange) => {
        let t = this._timeProvider.utcNow();
        let status = convertConnectivityStatus(s);
...
class CoinbaseOrderEntryGateway implements Interfaces.IOrderEntryGateway {
    ...
    sendOrder = (order: Models.OrderStatusReport) => {
        let cb = (err?: Error, resp?: any, ack?: CoinbaseOrderAck) => {
            let status: Models.OrderStatusUpdate;
            let t = this._timeProvider.utcNow();
    ...
    private onOpen = (tsMsg: Models.Timestamped<CoinbaseOpen>) => {
        let msg = tsMsg.data;
        let orderId = this._orderData.exchIdsToClientIds.get(msg.order_id);
    ...
    private onDone = (tsMsg: Models.Timestamped<CoinbaseDone>) => {
        ...
        let ordStatus = msg.reason === 'filled'
        ...
        this._authClient.getAccounts((err?: Error, resp?: any, data?: CoinbaseAccountInformation[]|{message: string}) => {
            try {
                if (Array.isArray(data)) {
                    _.forEach(data, d => {
                        let c = Models.toCurrency(d.currency);
                        let rpt = new Models.CurrencyPosition(convertPrice(d.available), convertPrice(d.hold), c);
