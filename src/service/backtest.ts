...
export class BacktestTimeProvider implements Utils.IBacktestingTimeProvider {
  constructor(
    private _internalTime: moment.Moment,
  ...
  private setAction = (
    ...
    let dueTime = this._internalTime.clone().add(time);
    ...
    this._timeouts.sort((a, b) =>
      Utils.fastDiff(a.time.toDate(), b.time.toDate())
    );
  };
  ...
  scrollTimeTo = (time: moment.Moment) => {
    ...
    while (
      ...
      Utils.fastDiff(_.first(this._timeouts).time.toDate(), time.toDate()) < 0
    ) {
      let evt: Timed = this._timeouts.shift();
...
export class BacktestGateway
  implements
    Interfaces.IPositionGateway,
    Interfaces.IOrderEntryGateway,
    Interfaces.IMarketDataGateway {
  ...
  cancelOrder = (cancel: Models.OrderStatusReport) => {
    this.timeProvider.setTimeout(() => {
      if (cancel.side === Models.Side.Bid) {
        let existing = this._openBidOrders[cancel.orderId];
        ...
        let existing = this._openAskOrders[cancel.orderId];
        if (typeof existing === 'undefined') {
          this.OrderUpdate.trigger({
            orderId: cancel.orderId,
            orderStatus: Models.OrderStatus.Rejected
  ...
  private onMarketData = (market: Models.Market) => {
    this._openAskOrders = this.tryToMatch(
      <any>_.values(this._openAskOrders),
      market.bids,
      Models.Side.Ask
    );
    this._openBidOrders = this.tryToMatch(
      <any>_.values(this._openBidOrders),
      market.asks,
      Models.Side.Bid
    );
  ...
  private tryToMatch = (
    orders: Models.OrderStatusReport[],
    ...
    let cmp = side === Models.Side.Ask ? (m, o) => o < m : (m, o) => o > m;
    _.forEach(orders, order => {
      _.forEach(marketSides, mkt => {
        if (
          (cmp(mkt.price, order.price) ||
            order.type === Models.OrderType.Market) &&
          order.quantity > 0
        ) {
          let px = order.price;
          if (order.type === Models.OrderType.Market) px = mkt.price;

          let update: Models.OrderStatusUpdate = {
            orderId: order.orderId,
            lastPrice: px
          };
    ...
    let liveOrders = _.filter(orders, o => o.quantity > 0);

    if (liveOrders.length > 5)
      console.warn(
        'more than 5 outstanding ' + Models.Side[side] + ' orders open'
      );
  ...
  private onMarketTrade = (trade: Models.MarketTrade) => {
    ...
    this.MarketTrade.trigger(
      new Models.GatewayMarketTrade(
        trade.price,
        trade.size,
        trade.time,
        false,
  ...
  public run = () => {
    ...
    _(this._inputData).forEach(i => {
      ...
      if (typeof i['make_side'] !== 'undefined') {
        ...
        typeof i['bids'] !== 'undefined' ||
        typeof i['asks'] !== 'undefined'
      ) {
...
export class BacktestPersister<T>
  implements Persister.ILoadAll<T>, Persister.ILoadLatest<T> {
  public load = (
    exchange: Models.Exchange,
...
let backtestServer = () => {
    ...
    process.on(reason, (e?) => {
      console.log(
        util.format(
          ...
          reason,
          e,
          typeof e !== 'undefined' ? e.stack : undefined
        )
      );
  ...
  let mdFile = process.env['MD_FILE'];
  let paramFile = process.env['PARAM_FILE'];
  let savedProgressFile =
    process.env['PROGRESS_FILE'] || 'nextParameters_saved.txt';
  let backtestResultFile = process.env['RESULT_FILE'] || 'backtestResults.txt';

  let rawParams = fs.readFileSync(paramFile, 'utf8');
  let parameters: BacktestParameters[] = JSON.parse(rawParams);
  if (fs.existsSync(savedProgressFile)) {
    let l = parseInt(fs.readFileSync(savedProgressFile, 'utf8'));
  ...
  app.post('/result', (req, res) => {
    let params = req.body;
    console.log(
      'Accept backtest results, volume =',
      params[2].volume.toFixed(2),
      'val =',
      params[1].value.toFixed(2),
      'qVal =',
      params[1].quoteValue.toFixed(2)
    );
