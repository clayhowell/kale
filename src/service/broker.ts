...
export class MarketDataBroker implements Interfaces.IMarketDataBroker {
  ...
  constructor(
    ...
    rawMarketPublisher.registerSnapshot(
      () => (this.currentBook === null ? [] : [this.currentBook])
    );
...
export class OrderBroker implements Interfaces.IOrderBroker {
  ...
  private _cancelsWaitingForExchangeOrderId: {
    [clId: string]: Models.OrderCancel;
  } = {};
  ...
  public updateOrderState = (
    ...
    if (osr.orderStatus === Models.OrderStatus.New) {
      ...
      if (typeof orig === 'undefined') {
        ...
        const secondChance = this._orderCache.exchIdsToClientIds.get(
          osr.exchangeId
        );
    ...
    const leavesQuantity = getOrFallback(
      osr.leavesQuantity,
      orig.leavesQuantity
    );
    ...
    if (typeof osr.cumQuantity !== 'undefined') {
      ...
      cumQuantity =
        getOrFallback(orig.cumQuantity, 0) + getOrFallback(osr.lastQuantity, 0);
    }
    ...
    const o: Models.OrderStatusReport = {
      ...
      computationalLatency:
        getOrFallback(osr.computationalLatency, 0) +
        getOrFallback(orig.computationalLatency, 0),
    ...
    if (
      !this._oeGateway.cancelsByClientOrderId &&
    ...
    if (osr.lastQuantity > 0) {
      ...
      if (typeof liq !== 'undefined') {
        ...
        feeCharged =
          liq === Models.Liquidity.Make
            ? this._baseBroker.makeFee()
            : this._baseBroker.takeFee();
  ...
  private orderStatusSnapshot = (): Models.OrderStatusReport[] => {
    return Array.from(this._orderCache.allOrders.values()).filter(
      this.shouldPublish
    );
  }
  ...
  constructor(
    ...
    private _submittedOrderReciever: Messaging.IReceive<
      Models.OrderRequestFromUI
    >,
    private _cancelOrderReciever: Messaging.IReceive<Models.OrderStatusReport>,
    private _cancelAllOrdersReciever: Messaging.IReceive<
      Models.CancelAllOrdersRequest
    >,
    private _messages: Messages.MessagesPubisher,
    ...
    _submittedOrderReciever.registerReceiver((o: Models.OrderRequestFromUI) => {
      ...
      try {
        const order = new Models.SubmitNewOrder(
          Models.Side[o.side],
          o.quantity,
    ...
    _cancelOrderReciever.registerReceiver(o => {
      ...
      try {
        this.cancelOrder(
          new Models.OrderCancel(o.orderId, o.exchange, _timeProvider.utcNow())
        );
      } catch (e) {
        this._log.error(e, 'unhandled exception while submitting order', o);
      }
    ...
    _cancelAllOrdersReciever.registerReceiver(o => {
      ...
      this.cancelOpenOrders().then(
        x => this._log.info('cancelled all ', x, ' open orders'),
...
export class PositionBroker implements Interfaces.IPositionBroker {
  ...
  private onPositionUpdate = (rpt: Models.CurrencyPosition) => {
    ...
    const mid =
      (this._mdBroker.currentBook.bids[0].price +
        this._mdBroker.currentBook.asks[0].price) /
      2.0;
    const baseValue =
      baseAmount +
      quoteAmount / mid +
      basePosition.heldAmount +
      quotePosition.heldAmount / mid;
    const quoteValue =
      baseAmount * mid +
      quoteAmount +
      basePosition.heldAmount * mid +
    ...
    if (
      ...
      Math.abs(positionReport.baseHeldAmount - this._report.baseHeldAmount) <
        2e-2 &&
      Math.abs(positionReport.quoteHeldAmount - this._report.quoteHeldAmount) <
        2e-2
    )
      return;
  ...
  constructor(
    ...
    this._positionPublisher.registerSnapshot(
      () => (this._report === null ? [] : [this._report])
    );
  }
}
...
export class ExchangeBroker implements Interfaces.IBroker {
  ...
  public onConnect = (
    ...
    const newStatus =
      this.mdConnected === Models.ConnectivityStatus.Connected &&
      this.oeConnected === Models.ConnectivityStatus.Connected
    ...
    this._log.info(
      'Connection status changed :: %s :: (md: %s) (oe: %s)',
      Models.ConnectivityStatus[this._connectStatus],
      Models.ConnectivityStatus[this.mdConnected],
      Models.ConnectivityStatus[this.oeConnected]
    );
  ...
  constructor(
    ...
    private _oeGateway: Interfaces.IOrderEntryGateway,
    private _connectivityPublisher: Messaging.IPublish<
