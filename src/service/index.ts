...
const defaultActive: Models.SerializedQuotesActive = new Models.SerializedQuotesActive(
  false,
  new Date(1)
);
...
const backTestSimulationSetup = (
  ...
  const timeProvider: Utils.ITimeProvider = new Backtest.BacktestTimeProvider(
  ...
  const getExch = async (
    orderCache: Broker.OrderStateCache
  ): Promise<Interfaces.CombinedGateway> => new Backtest.BacktestExchange(gw);

  const getPublisher = <T>(
    topic: string,
    persister?: Persister.ILoadAll<T>
  ): Messaging.IPublish<T> => {
    return new Messaging.NullPublisher<T>();
  };

  const getReceiver = <T>(topic: string): Messaging.IReceive<T> =>
    new Messaging.NullReceiver<T>();

  const getPersister = <T>(
    collectionName: string
  ): Promise<Persister.ILoadAll<T>> =>
    new Promise(cb => cb(new Backtest.BacktestPersister<T>()));

  const getRepository = <T>(
    defValue: T,
    collectionName: string
  ): Promise<Persister.ILoadLatest<T>> =>
    new Promise(cb => cb(new Backtest.BacktestPersister<T>([defValue])));

  const startingActive: Models.SerializedQuotesActive = new Models.SerializedQuotesActive(
    true,
    timeProvider.utcNow()
  );
  const startingParameters: Models.QuotingParameters =
...
const liveTradingSetup = (): SimulationClasses => {
  ...
  const getPublisher = <T>(
    ...
    const socketIoPublisher = new Messaging.Publisher<T>(
      topic,
      io,
      null,
    ...
    if (persister)
      return new Web.StandaloneHttpPublisher<T>(
        socketIoPublisher,
        topic,
        app,
        persister
  ...
  const getPersister = async <T extends Persister.Persistable>(
    collectionName: string
  ): Promise<Persister.ILoadAll<T>> => {
    const coll = await (await db).collection(collectionName);
    return new Persister.Persister<T>(
      timeProvider,
      coll,
      collectionName,
      exchange,
  ...
  const getRepository = async <T extends Persister.Persistable>(
    defValue: T,
    collectionName: string
  ): Promise<Persister.ILoadLatest<T>> =>
    new Persister.RepositoryPersister<T>(
      await (await db).collection(collectionName),
      defValue,
      collectionName,
      exchange,
      pair
...
interface SimulationClasses {
  ...
  getPersister<T extends Persister.Persistable>(
    collectionName: string
  ): Promise<Persister.ILoadAll<T>>;
  getRepository<T>(
    defValue: T,
...
const runTradingSystem = async (classes: SimulationClasses): Promise<void> => {
  ...
  const tbpPersister = await getPersister<Models.TargetBasePositionValue>(
    'tbp'
  );
  const tsvPersister = await getPersister<Models.TradeSafety>('tsv');
  const marketDataPersister = await getPersister<Models.Market>(
    Messaging.Topics.MarketData
  );

  const activePersister = await classes.getRepository<
    Models.SerializedQuotesActive
  >(classes.startingActive, Messaging.Topics.ActiveChange);
  const paramsPersister = await classes.getRepository<Models.QuotingParameters>(
  ...
  const quotingParametersReceiver = getReceiver<Models.QuotingParameters>(
    Messaging.Topics.QuotingParametersChange
  );
  const submitOrderReceiver = getReceiver<Models.OrderRequestFromUI>(
    Messaging.Topics.SubmitNewOrder
  );
  const cancelOrderReceiver = getReceiver<Models.OrderStatusReport>(
    Messaging.Topics.CancelOrder
  );
...
const harness = async (): Promise<any> => {
  if (config.inBacktestMode) {
    ...
    const input = await getFromBacktestServer('inputData').then(body => {
      const inp: Array<Models.Market | Models.MarketTrade> =
        typeof body === 'string' ? eval(body) : body; // tslint:disable-line
    ...
    const nextParameters = (): Promise<Backtest.BacktestParameters> =>
      getFromBacktestServer('nextParameters').then(body => {
        const p =
          typeof body === 'string'
            ? <string | Backtest.BacktestParameters>JSON.parse(body)
            : body;
