...
export class SafetyCalculator {
  ...
  public set latest(val: Models.TradeSafety) {
    if (
      !this._latest ||
      Math.abs(val.combined - this._latest.combined) > 1e-3 ||
  ...
  private isOlderThan(o: Models.Trade, settings: Models.QuotingParameters) {
    let now = this._timeProvider.utcNow();
    return (
      Math.abs(Utils.fastDiff(now, o.time)) > 1000 * settings.tradeRateSeconds
    );
  }

  private computeQtyLimit = () => {
    let settings = this._repo.latest;
    ...
    for (let ti = this._broker._trades.length - 1; ti > -1; ti--) {
      if (
        this._broker._trades[ti].side === Models.Side.Bid &&
        buyPq < settings.size
      ) {
        _buyPq = Math.min(
          settings.size - buyPq,
          this._broker._trades[ti].quantity
        );
        buyPing += this._broker._trades[ti].price * _buyPq;
        buyPq += _buyPq;
      }
      if (
        this._broker._trades[ti].side === Models.Side.Ask &&
        sellPq < settings.size
      ) {
        _sellPq = Math.min(
          settings.size - sellPq,
          this._broker._trades[ti].quantity
        );
    ...
    while (_.size(this._buys) > 0 && _.size(this._sells) > 0) {
      let sell = _.last(this._sells);
      let buy = _.last(this._buys);
      if (sell.price >= buy.price) {
        let sellQty = sell.quantity;
        let buyQty = buy.quantity;
    ...
    let computeSafety = (t: Models.Trade[]) =>
      t.reduce((sum, t) => sum + t.quantity, 0) / this._qlParams.latest.size;

    this.latest = new Models.TradeSafety(
      computeSafety(this._buys),
      computeSafety(this._sells),
