...
export class StandaloneHttpPublisher<T> {
    constructor(
        ...
        _httpApp.get('/data/' + route, (req: express.Request, res: express.Response) => {
            let getParameter = <T>(pName: string, cvt: (r: string) => T) => {
                let rawMax: string = req.param(pName, null);
                return (rawMax === null ? null : cvt(rawMax));
            };

            let max = getParameter<number>('max', r => parseInt(r));
            let startTime = getParameter<moment.Moment>('start_time', r => moment(r));
