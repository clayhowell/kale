...
let mypopover = ($compile: ng.ICompileService, $templateCache: ng.ITemplateCacheService) => {
    let getTemplate = (contentType, template_url) => {
        let template = '';
        switch (contentType) {
            case 'user':
    ...
    return {
        ...
        link: (scope, element, attrs) => {
            let popOverContent = $compile('<div>' + getTemplate('user', attrs.popoverTemplate) + '</div>')(scope);
            let options = {
...
export let sharedDirectives = 'sharedDirectives';
