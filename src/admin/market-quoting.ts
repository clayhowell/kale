...
let MarketQuotingController = ($scope: MarketQuotingScope,
        $log: ng.ILogService,
        subscriberFactory: Shared.SubscriberFactory,
        product: Shared.ProductState) => {

    let toPrice = (px: number): string => px.toFixed(product.fixed);
    let toPercent = (askPx: number, bidPx: number): string => ((askPx - bidPx) / askPx * 100).toFixed(2);
    ...
    let updateMarket = (update: Models.Market) => {
        if (update == null) {
            clearMarket();
            return;
        }
    ...
    let updateQuote = (quote: Models.TwoSidedQuote) => {
        if (quote !== null) {
            if (quote.bid !== null) {
    ...
    let updateQuoteStatus = (status: Models.TwoSidedQuoteStatus) => {
        if (status == null) {
            clearQuoteStatus();
            return;
        }
    ...
    let updateFairValue = (fv: Models.FairValue) => {
        if (fv == null) {
            clearFairValue();
            return;
        }
    ...
    let makeSubscriber = <T>(topic: string, updateFn, clearFn) => {
        let sub = subscriberFactory.getSubscriber<T>($scope, topic)
...
export let marketQuotingDirective = 'marketQuotingDirective';
