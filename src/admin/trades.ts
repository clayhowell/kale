...
let TradesListController = ($scope: TradesScope, $log: ng.ILogService, subscriberFactory: Shared.SubscriberFactory, uiGridConstants: any) => {
    ...
    let addTrade = t => $scope.trade_statuses.push(new DisplayTrade(t));

    let sub = subscriberFactory.getSubscriber($scope, Messaging.Topics.Trades)
...
let tradeList = (): ng.IDirective => {
    let template = '<div><div ui-grid="gridOptions" ui-grid-grouping class="table table-striped table-hover table-condensed" style="height: 553px" ></div></div>';
...
export let tradeListDirective = 'tradeListDirective';
