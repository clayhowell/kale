...
class QuotingButtonViewModel extends FormViewModel<boolean> {
  constructor(
    sub: Messaging.ISubscribe<boolean>,
...
class DisplayQuotingParameters extends FormViewModel<Models.QuotingParameters> {
  ...
  constructor(
    sub: Messaging.ISubscribe<Models.QuotingParameters>,
    fire: Messaging.IFire<Models.QuotingParameters>
  ) {
    super(
      new Models.QuotingParameters(
        null,
        null,
        null,
        null,
    ...
    this.availableQuotingModes = DisplayQuotingParameters.getMapping(
      Models.QuotingMode
    );
    this.availableFvModels = DisplayQuotingParameters.getMapping(
      Models.FairValueModel
    );
    this.availableAutoPositionModes = DisplayQuotingParameters.getMapping(
      Models.AutoPositionMode
    );
  }
...
export class DisplayPair {
  ...
  constructor(
    ...
    const connectivitySubscriber = subscriberFactory
      .getSubscriber(scope, Messaging.Topics.ExchangeConnectivity)
      .registerSubscriber(setExchangeStatus, cs =>
        cs.forEach(setExchangeStatus)
      )
    ...
    const activeSub = subscriberFactory.getSubscriber<boolean>(
      scope,
    ...
    const qpSub = subscriberFactory.getSubscriber<Models.QuotingParameters>(
      scope,
