...
let MarketTradeGrid = ($scope: MarketTradeScope,
                       $log: ng.ILogService,
    ...
    let addNewMarketTrade = (u: Models.MarketTrade) => {
        if (u != null)
            $scope.marketTrades.push(new MarketTradeViewModel(u));
    };

    let sub = subscriberFactory.getSubscriber($scope, Messaging.Topics.MarketTrade)
...
export let marketTradeDirective = 'marketTradeDirective';

angular
    .module(marketTradeDirective, ['ui.bootstrap', 'ui.grid', Shared.sharedDirectives])
    .directive('marketTradeGrid', () => {
        let template = '<div><div style="height: 553px" class="table table-striped table-hover table-condensed" ui-grid="marketTradeOptions"></div></div>';
