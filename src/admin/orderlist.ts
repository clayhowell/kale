...
class DisplayOrderStatusReport {
    ...
    private static getOrderStatus(o: Models.OrderStatusReport): string {
        let endingModifier = (o: Models.OrderStatusReport) => {
            if (o.pendingCancel)
                return ', PndCxl';
...
let OrderListController = ($scope: OrderListScope,
                           $log: ng.ILogService,
                           subscriberFactory: Shared.SubscriberFactory,
                           fireFactory: Shared.FireFactory,
                           uiGridConstants: any) => {
    let fireCxl = fireFactory.getFire(Messaging.Topics.CancelOrder);
    ...
    let sub = subscriberFactory.getSubscriber($scope, Messaging.Topics.OrderStatusReports)
...
let orderList = (): ng.IDirective => {
    let template = '<div><div ui-grid="gridOptions" ui-grid-grouping class="table table-striped table-hover table-condensed" style="height: 150px"></div></div>';
...
export let orderListDirective = 'orderListDirective';
