...
let MessagesController = ($scope: MessageLoggerScope, $log: ng.ILogService, subscriberFactory: Shared.SubscriberFactory) => {
    $scope.messages = [];
    ...
    let addNewMessage = (u: Models.Message) => {
        $scope.messages.push(new MessageViewModel(u));
    };

    let sub = subscriberFactory.getSubscriber($scope, Messaging.Topics.Message)
...
export let messagesDirective = 'messagesDirective';

angular
    .module(messagesDirective, ['ui.bootstrap', 'ui.grid', Shared.sharedDirectives])
    .directive('messagesGrid', () => {
        let template = '<div><div style="height: 75px" class="table table-striped table-hover table-condensed" ui-grid="messageOptions"></div></div>';
